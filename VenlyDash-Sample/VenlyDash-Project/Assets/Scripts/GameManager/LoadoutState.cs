...
public class LoadoutState : AState
{
    ...
    protected Dictionary<int, int> _characterUsedAccessoryLUT = new Dictionary<int, int>();
    ...
    protected const string k_WalletSceneName = "Wallet";
    ...
    {
        tutorialBlocker.SetActive(!PlayerDataWeb3.instance.tutorialDone);
        ...
        if(m_PowerupToUse != Consumable.ConsumableType.NONE)
        {
            ...
            if (!PlayerDataWeb3.instance.consumables.ContainsKey(m_PowerupToUse) || PlayerDataWeb3.instance.consumables[m_PowerupToUse] == 0)
    ...
    public override void Exit(AState to)
    {
        ...
        if (gs != null)
        {
			...
			if (m_PowerupToUse != Consumable.ConsumableType.NONE)
			{
                PlayerDataWeb3.instance.Consume(m_PowerupToUse);
    ...
    public override void Tick()
    {
		...
		charSelect.gameObject.SetActive(PlayerDataWeb3.instance.characters.Count > 1);
		themeSelect.gameObject.SetActive(PlayerDataWeb3.instance.themes.Count > 1);
    }
    ...
    {
        UnityEngine.SceneManagement.SceneManager.LoadScene(k_WalletSceneName, UnityEngine.SceneManagement.LoadSceneMode.Additive);
    }

    public void ChangeCharacter(int dir)
    {
        _characterUsedAccessoryLUT[PlayerDataWeb3.instance.usedCharacter] = PlayerDataWeb3.instance.usedAccessory;

        PlayerDataWeb3.instance.usedCharacter += dir;
        if (PlayerDataWeb3.instance.usedCharacter >= PlayerDataWeb3.instance.characters.Count)
            PlayerDataWeb3.instance.usedCharacter = 0;
        else if(PlayerDataWeb3.instance.usedCharacter < 0)
            PlayerDataWeb3.instance.usedCharacter = PlayerDataWeb3.instance.characters.Count-1;

        if (_characterUsedAccessoryLUT.ContainsKey(PlayerDataWeb3.instance.usedCharacter))
            PlayerDataWeb3.instance.usedAccessory = _characterUsedAccessoryLUT[PlayerDataWeb3.instance.usedCharacter];
        else PlayerDataWeb3.instance.usedAccessory = -1;
    ...
    public void ChangeAccessory(int dir)
    {
        ...
        if (m_UsedAccessory != -1)
            PlayerDataWeb3.instance.usedAccessory = m_OwnedAccesories[m_UsedAccessory];
        else
            PlayerDataWeb3.instance.usedAccessory = -1;
    ...
    {
        PlayerDataWeb3.instance.usedTheme += dir;
        if (PlayerDataWeb3.instance.usedTheme >= PlayerDataWeb3.instance.themes.Count)
            PlayerDataWeb3.instance.usedTheme = 0;
        else if (PlayerDataWeb3.instance.usedTheme < 0)
            PlayerDataWeb3.instance.usedTheme = PlayerDataWeb3.instance.themes.Count - 1;
    ...
    public IEnumerator PopulateTheme()
    {
        ...
        while (t == null)
        {
            t = ThemeDatabase.GetThemeData(PlayerDataWeb3.instance.themes[PlayerDataWeb3.instance.usedTheme]);
            yield return null;
        }
    ...
    public IEnumerator PopulateCharacters()
    {
        ...
        if (!m_IsLoadingCharacter)
        {
            ...
            while (newChar == null)
            {
                Character c = CharacterDatabase.GetCharacter(PlayerDataWeb3.instance.characters[PlayerDataWeb3.instance.usedCharacter]);
                ...
                if (c != null)
                {
                    ...
                    for (int i = 0; i < c.accessories.Length; ++i)
                    {
                        ...
                        if (PlayerDataWeb3.instance.characterAccessories.Contains(compoundName))
                        {
    ...
    void SetupAccessory()
    {
        ...
        //Select Desired Accessory if available
        if (!string.IsNullOrEmpty(PlayerDataWeb3.instance.desiredAccessory))
        {
            PlayerDataWeb3.instance.usedAccessory = -1;
            for (var i = 0; i < c.accessories.Length; i++)
            {
                if (PlayerDataWeb3.instance.desiredAccessory.Contains(c.accessories[i].accessoryName))
                {
                    PlayerDataWeb3.instance.usedAccessory = i;
                    break;
                }
            }

            PlayerDataWeb3.instance.desiredAccessory = null;
        }
        
        c.SetupAccesory(PlayerDataWeb3.instance.usedAccessory);

        if (PlayerDataWeb3.instance.usedAccessory == -1)
        {
        ...
        else
        {
			...
			accesoryNameDisplay.text = c.accessories[PlayerDataWeb3.instance.usedAccessory].accessoryName;
			accessoryIconDisplay.sprite = c.accessories[PlayerDataWeb3.instance.usedAccessory].accessoryIcon;
        }
    }
	...
	void PopulatePowerup()
	{
        ...
        if (PlayerDataWeb3.instance.consumables.Count > 0)
        {
            ...
            if (c != null)
            {
                ...
                powerupCount.text = PlayerDataWeb3.instance.consumables[m_PowerupToUse].ToString();
            }
            else
            {
	...
	public void ChangeConsumable(int dir)
	{
		...
		do
		{
			...
			int count = 0;
			if(PlayerDataWeb3.instance.consumables.TryGetValue((Consumable.ConsumableType)m_UsedPowerupIndex, out count) && count > 0)
			{
				found = true;
			}
    ...
    {
        if (PlayerDataWeb3.instance.tutorialDone)
        {
            if (PlayerDataWeb3.instance.ftueLevel == 1)
            {
                PlayerDataWeb3.instance.ftueLevel = 2;
