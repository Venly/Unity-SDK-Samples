...
public class ShopAccessoriesList : ShopList
{
    ...
    void LoadedAccessory(AsyncOperationHandle<GameObject> op, int characterIndex, int accessoryIndex)
    {
	    ...
	    else
	    {
		    ...
		    itm.buyButton.onClick.AddListener(()=>
            {
	...
	{
		if (accessory.cost > PlayerDataWeb3.instance.User.Coins)
		{
		...
		//if (accessory.premiumCost > PlayerDataWeb3.instance.premium)
		//{
		//	itm.buyButton.interactable = false;
    ...
    {
        Web3Controller.ShowLoader("Claiming Accessory");
        Web3Controller.ClaimAccessory(name, cost)
