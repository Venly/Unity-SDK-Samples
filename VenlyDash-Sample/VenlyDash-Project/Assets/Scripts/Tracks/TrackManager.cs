...
public class TrackManager : MonoBehaviour
{
    ...
    public IEnumerator Begin()
    {
        if (!m_Rerun)
        {
            ...
            // Spawn the player
            var op = Addressables.InstantiateAsync(PlayerDataWeb3.instance.characters[PlayerDataWeb3.instance.usedCharacter],
                Vector3.zero,
                Quaternion.identity);
            yield return op;
            if (op.Result == null || !(op.Result is GameObject))
            {
                Debug.LogWarning(string.Format("Unable to load character {0}.", PlayerDataWeb3.instance.characters[PlayerDataWeb3.instance.usedCharacter]));
                yield break;
            }
            Character player = op.Result.GetComponent<Character>();

            player.SetupAccesory(PlayerDataWeb3.instance.usedAccessory);
            ...
            if (m_IsTutorial)
                ...
                m_CurrentThemeData = ThemeDatabase.GetThemeData(PlayerDataWeb3.instance.themes[PlayerDataWeb3.instance.usedTheme]);
            ...
            PlayerDataWeb3.instance.StartRunMissions(this);
    ...
    public void End()
    {
        ...
        {
            PlayerDataWeb3.instance.AddConsumable(characterController.inventory.GetConsumableType());
    ...
    void Update()
    {
        ...
        if (!m_IsTutorial)
        {
            ...
            int currentTarget = (PlayerDataWeb3.instance.rank + 1) * 300;
            if (m_TotalWorldDistance > currentTarget)
            {
                PlayerDataWeb3.instance.rank += 1;
...
//"level" in our game are milestone the player have to reach : one every 300m
            ...
            PlayerDataWeb3.instance.UpdateMissions(this);
        }
